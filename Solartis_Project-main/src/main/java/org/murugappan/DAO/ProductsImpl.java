...
public class ProductsImpl implements ProductsDAO {
    ...
    @Override
    public void addProducts(String productname,String description,int costprice,int sellingprice,int stock) {

        try {
            preparedStatement = con.prepareStatement("INSERT INTO product_details (product_name, product_des, cost_price, selling_price, Stock) VALUES(?,?,?,?,?)"
       );
	...
	public void showProducts() {
		try {
			 ...
			 System.out.printf("%-15s%-20s%-20s%-15s%-15s%-10s%n", 
			 ...
			 while (rs.next()) {
	                ...
	                System.out.printf("%-15d%-20s%-25s%-15d%-15d%-10d%n", 
	                    productId, productName, productDes, costPrice, sellingPrice, stock);
	            }
